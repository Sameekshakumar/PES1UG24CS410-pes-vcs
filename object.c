...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    // step 3: deduplication — already stored? nothing to do
    ...
    // step 5 & 6: write to a temp file, fsync it
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    if ((long)fread(raw, 1, file_size, f) != file_size) { free(raw); fclose(f); return -1; }
    fclose(f);

    // step 4: verify integrity — rehash and compare
    ObjectID computed;
    ...
    // step 6: copy just the data portion (after the '\0')
